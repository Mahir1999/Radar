...
# --- إعدادات الصفحة ---
st.set_page_config(page_title="Ultra Radar v47", page_icon="⚡", layout="centered")
...
st.title("⚡ الرادار الذكي المتكامل v47")

# --- 1️⃣ لوحة التحكم المالية (بديلة للشريط الجانبي) ---
col_net, col_count, col_reset = st.columns([2, 1, 1])
with col_net:
    color = "#39ff14" if st.session_state.total_net >= 0 else "#ff4b4b"
    st.markdown(f"""<div class="stat-card">صافي الربح الكلي<br>
    <b style="color:{color}; font-size:24px;">{st.session_state.total_net}</b></div>""", unsafe_allow_html=True)
with col_count:
    st.markdown(f"""<div class="stat-card">الجولات<br>
    <b style="font-size:24px;">{len(st.session_state.history)}</b></div>""", unsafe_allow_html=True)
with col_reset:
...
# --- 2️⃣ تحليل الاحتمالات والعنصر الذهبي ---
...
if len(hist) >= 20:
    ...
    if next_options:
        ...
        st.info("🔄 بانتظار تكرار النمط...")
else:
    st.warning(f"📡 متبقي {20 - len(hist)} جولة لتفعيل التوقع.")

# --- 3️⃣ إدارة مبالغ الرهان الفردية ---
st.markdown('<div class="section-title">📝 مبالغ الرهان الحالية</div>', unsafe_allow_html=True)
def label_style(code):
    return f"🌟 {SYMBOLS[code]['name']}" if code == top_candidate else SYMBOLS[code]['name']

c1, c2, c3 = st.columns(3)
b5 = c1.number_input(label_style(5), 0, 5000, 0, 5, key="b5")
b7 = c2.number_input(label_style(7), 0, 5000, 0, 5, key="b7")
b6 = c3.number_input(label_style(6), 0, 5000, 0, 5, key="b6")
