...
# --- إعدادات الصفحة ---
st.set_page_config(page_title="Greedy AI v94.2", page_icon="🛡️", layout="centered")

st.markdown("""
    <style>
    .block-container { padding-top: 1rem; padding-bottom: 1rem; }
    .stButton>button { width: 100%; height: 48px; font-weight: bold; border-radius: 10px; }
    .next-hit-card { background: #1a1a1a; border: 2px solid #39ff14; padding: 15px; border-radius: 15px; text-align: center; margin-bottom: 10px; }
    ...
    /* شريط النمط الجديد */
    .pattern-bar { background: #222; border-left: 5px solid #ffaa00; padding: 10px; border-radius: 8px; text-align: center; margin: 10px 0; }
    </style>
...
if total_h > 0:
    ...
    st.markdown(f'<div class="next-hit-card"><div style="color:#39ff14; font-size:12px; font-weight:bold;">🎯 المربع الذهبي</div><div class="quad-box">{"".join([f'<div class="quad-item">{SYMBOLS[c].split()[0]}</div>' for c in top_4])}</div></div>', unsafe_allow_html=True)
    st.markdown(f'<div class="insurance-card"><div style="color:#00aaff; font-size:12px; font-weight:bold;">🛡️ درع التأمين</div><div style="color:white; font-size:20px; font-weight:bold;">{SYMBOLS[insurance_slot].split()[0]}</div></div>', unsafe_allow_html=True)

# --- 3. شريط النمط (إجباري الظهور) ---
...
<div class="pattern-bar">
    <div style="font-size:14px; color:{p_color}; font-weight:bold;">الحالة: {p_status}</div>
    <div style="font-size:11px; color:#aaa;">🧠 أنماط مكتشفة: {st.session_state.patterns_found} | 💰 جكبوت: {gap_9}</div>
</div>
""", unsafe_allow_html=True)
