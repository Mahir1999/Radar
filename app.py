...
# --- 1. الإعدادات ---
st.set_page_config(page_title="Greedy AI v105.4", page_icon="💎", layout="centered")

# --- 2. تهيئة الذاكرة الشاملة ---
if 'history' not in st.session_state:
    st.session_state.update({
        'history': [], 'hits': 0, 'misses': 0, 'p_count': 0,
        'preds_history': [], 'action_hit': [], 'max_streak': 0,
        'cur_streak': 0, 'balance': 0, 'fingerprint': "بدء التحليل..."
    })

SYMBOLS = {1: "🍅", 2: "🌽", 3: "🥕", 4: "🫑", 5: "🐔", 6: "🐑", 7: "🐟", 8: "🦐", 9: "💰"}
MULT = {1: 5, 2: 5, 3: 5, 4: 5, 5: 45, 6: 15, 7: 25, 8: 10, 9: 0}

# --- 3. المحرك المالي المتقدم ---
def register_result(code, b_q, b_i):
    # نأخذ التوقعات التي كانت ظاهرة لحظة الضغط
...
# --- 4. محرك التوقعات الدقيق (الميزان + الوزن الزمني) ---
hist = st.session_state.history; total_h = len(hist)
if total_h > 0:
    # ميزة الوزن الزمني (Time-Decay)
    scores = {c: ((hist[-5:].count(c) * 3.0 + hist[-15:].count(c)) * 0.75 + (list(reversed(hist)).index(c) if c in hist else total_h) * 0.25) for c in range(1, 9)}
    # ميزة الميزان الرقمي (Balance)
    if sum(1 for p in (st.session_state.preds if 'preds' in st.session_state else [1,2,3,4])[:4] if p >= 5) >= 3 or hist[-1] >= 5:
...
# --- 5. الواجهة (التصميم المفصل المعتمد) ---
st.markdown("""<style>
...
st.markdown(f'<div class="main-box"><div style="color:#39ff14; font-size:11px; font-weight:bold; margin-bottom:5px;">🎯 المربع الذهبي (v105.4 متكامل)</div>'
...
# أزرار التحكم وإصلاح التراجع المالي
c1, c2, c3, c4 = st.columns([0.8, 1, 1, 1])
if c1.button("↩️"):
    if st.session_state.history:
        l_code = st.session_state.history.pop()
        l_status = st.session_state.action_hit.pop()
        l_preds = st.session_state.preds_history.pop()
        if len(st.session_state.history) >= 29:
            cost = (bq * 4) + bi
            is_q = l_code in l_preds[:4]; is_i = (len(l_preds) > 4 and l_code == l_preds[4])
            win = (bq * MULT[l_code]) if is_q else ((bi * MULT[l_code]) if is_i else 0)
            st.session_state.balance -= (win - cost)
        if l_status: st.session_state.hits -= 1
