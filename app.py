...
# --- إعدادات الصفحة ---
st.set_page_config(page_title="Greedy AI v98.4", page_icon="💎", layout="centered")
...
def register_result(code, bet_quad, bet_ins):
    ...
    # حساب الأنماط (قبل المسح)
    h = st.session_state.history
    if len(h) >= 2:
        if h[-2:] == h[:-2][-2:]: st.session_state.p_count += 1
...
# --- التنسيق (CSS) عودة المربعات المفقودة ---
st.markdown("""
    <style>
...
# --- 1. الإدارة والعدادات ---
with st.expander("💰 إعدادات الرهان", expanded=(total_h >= 30)):
...
# --- 2. الحالة المالية ---
if total_h < 30:
    st.info(f"⏳ وضع الإحماء: سجل {30 - total_h} جولات لفتح المحفظة")
else:
