...
st.set_page_config(page_title="Greedy AI v103.0", page_icon="🕵️", layout="centered")

# --- 2. تهيئة الذاكرة (مع إضافة مفتاح البصمة) ---
...
for key in keys:
    if key not in st.session_state:
        ...
        elif key == 'fingerprint': st.session_state[key] = "تحليل البصمة..."
        else: st.session_state[key] = 0

SYMBOLS = {1: "🍅", 2: "🌽", 3: "🥕", 4: "🫑", 5: "🐔", 6: "🐑", 7: "🐟", 8: "🦐", 9: "💰"}
MULT = {1: 5, 2: 5, 3: 5, 4: 5, 5: 45, 6: 15, 7: 25, 8: 10, 9: 0}

# --- 3. المحرك المطور (مع ميزة البصمة) ---
def register_result(code, bet_q, bet_i):
    ...
    # ميزة البصمة: تحليل دوري للسيرفر
    if len(h) > 10:
        meat_ratio = sum(1 for x in h[-10:] if x >= 5) / 10
        if meat_ratio > 0.6: st.session_state.fingerprint = "بصمة: لحوم مكثفة 🍖"
        elif meat_ratio < 0.3: st.session_state.fingerprint = "بصمة: هدوء خضار 🥗"
        else: st.session_state.fingerprint = "بصمة: سيرفر متوازن ⚖️"

    # حساب الأنماط (البحث التاريخي)
    if len(h) >= 2:
        last_pair = [h[-1], code]
...
if total_h == 0:
    ...
    # الميزان الرقمي المتوازن
    if hist[-1] >= 5:
        for i in range(5, 9): scores[i] *= 0.6 
        for i in range(1, 5): scores[i] *= 1.4 
...
# --- 5. الواجهة (التصميم الأصلي المستقر) ---
st.markdown("""<style>
    ...
    .fingerprint-tag { background: #002233; color: #00aaff; border: 1px dashed #00aaff; border-radius: 5px; font-size: 10px; padding: 3px; margin-top: 5px; font-weight: bold; }
...
st.markdown(f'<div class="main-box"><div style="color:#39ff14; font-size:11px; font-weight:bold; margin-bottom:5px;">🎯 المربع الذهبي المتكامل</div>'
            ...
            '</div>'
            f'<div class="fingerprint-tag">{st.session_state.fingerprint}</div></div>', unsafe_allow_html=True)
